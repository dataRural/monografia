...
No desenvolvimento do sistema, modelos de linguagem foram utilizados como apoio à geração de código, à resolução de dúvidas técnicas e à exploração de alternativas de implementação. Os trechos sugeridos foram avaliados, adaptados e integrados ao projeto conforme as decisões de arquitetura definidas pelos autores. Na construção da interface, as ferramentas também auxiliaram na prototipação de componentes visuais e na definição de estilos.
...
\begin{compactitem}
    \item \textbf{Capítulo~\ref{chp:FUNDAMENTACAO} --- Gestão e Compartilhamento de Dados de Pesquisa:} discute a problemática que motiva este trabalho, abordando os desafios da gestão de dados em instituições de pesquisa, os princípios FAIR, dados abertos e licenciamento, versionamento de dados, plataformas de compartilhamento e as particularidades do formato CSV;

    \item \textbf{Capítulo~\ref{chp:PROPOSTA} --- DataRural:} apresenta a proposta da plataforma, compara a solução com plataformas existentes (Kaggle, Zenodo, Figshare), demonstra os principais fluxos e interfaces do sistema, define os requisitos, o controle de acesso e a modelagem de dados;

    \item \textbf{Capítulo~\ref{chp:EXPERIMENTOS} --- Implementação:} descreve a arquitetura adotada, justifica as decisões tecnológicas que motivaram cada escolha e detalha a implementação dos principais módulos do sistema;

    \item \textbf{Capítulo~\ref{chp:CONCLUSAO} --- Conclusão:} apresenta as considerações finais, as limitações identificadas e as possibilidades de continuidade e ampliação do trabalho.
\end{compactitem}