...
O compartilhamento por meios genéricos também não atende, de forma integrada, às diferentes situações encontradas no ambiente acadêmico. Alguns conjuntos devem ser divulgados publicamente, enquanto outros precisam permanecer restritos durante determinada etapa do trabalho. A sigla FAIR corresponde aos termos \textit{findable} (localizável), \textit{accessible} (acessível), \textit{interoperable} (interoperável) e \textit{reusable} (reutilizável). Esses princípios orientam a descrição e a disponibilização de dados de forma a favorecer sua descoberta e reutilização \cite{wilkinson2016fair}. Esses princípios não exigem que todos os dados sejam abertos: a acessibilidade pode envolver procedimentos de autenticação e autorização, desde que as condições de acesso estejam documentadas. Em atividades desenvolvidas por laboratórios, equipes ou unidades, ainda é necessário distinguir as responsabilidades de cada participante. Desse modo, uma solução institucional precisa conciliar descoberta, colaboração e controle, sem reduzir o conjunto de dados a um arquivo isolado de seu contexto.
...
\section{Uso de Inteligência Artificial}\label{sec:uso_ia}

O desenvolvimento deste trabalho utilizou ferramentas baseadas em modelos de linguagem de grande escala (\textit{large language models}) em diferentes etapas do processo. A presente seção registra essa utilização de forma transparente, em atendimento às boas práticas de integridade acadêmica.

Na escrita do texto, modelos de linguagem foram empregados como auxílio à redação, revisão de coesão e sugestão de reformulações. A responsabilidade pelo conteúdo, pelas decisões conceituais e pela verificação das informações permaneceu integralmente com os autores. Na busca de referências, as ferramentas auxiliaram na identificação de termos, autores e publicações relacionadas ao tema, servindo como ponto de partida para consultas posteriores em bases acadêmicas.

No desenvolvimento do sistema, modelos de linguagem foram utilizados como apoio à geração de código, à resolução de dúvidas técnicas e à exploração de alternativas de implementação. Os trechos sugeridos foram avaliados, adaptados e integrados ao projeto conforme as decisões de arquitetura definidas pelos autores. Na concepção da interface, o Claude auxiliou na geração de um mockup a partir do minimundo do projeto, com análise e feedback dos autores para orientar seu refinamento. A Seção~\ref{sec:prototipacao} apresenta esse processo e distingue a proposta visual das interfaces do sistema implementado.
...
\begin{compactitem}
    \item Capítulo~\ref{chp:FUNDAMENTACAO} (Gestão e Compartilhamento de Dados de Pesquisa): discute a problemática que motiva este trabalho, abordando os desafios da gestão de dados em instituições de pesquisa, os princípios FAIR, dados abertos e licenciamento, versionamento de dados, plataformas de compartilhamento e as particularidades do formato CSV;

    \item Capítulo~\ref{chp:PROPOSTA} (DataRural): apresenta a proposta da plataforma, compara a solução com plataformas existentes, discute a concepção da interface por meio de um mockup e define os fluxos de uso previstos, os requisitos, o controle de acesso e a modelagem de dados;

    \item Capítulo~\ref{chp:EXPERIMENTOS} (Implementação): descreve a arquitetura adotada, justifica as decisões tecnológicas e apresenta as funcionalidades implementadas, suas interfaces e sua relação com os requisitos da proposta;

    \item Capítulo~\ref{chp:CONCLUSAO} (Conclusão): apresenta as considerações finais, as limitações identificadas e as possibilidades de continuidade e ampliação do trabalho.
\end{compactitem}