...
A concepção da interface incluiu a elaboração de um \textit{mockup}, utilizado como representação visual inicial da plataforma. Nessa etapa, o objetivo foi explorar a organização das informações e os caminhos de navegação antes de sua implementação. O protótipo permitiu discutir como apresentar um dataset como um recurso documentado, com contexto, responsáveis e condições de reutilização, em vez de restringir sua apresentação a um arquivo disponível para download.
...
\begin{figure}[!htbp]
    ...
    \centerline{\footnotesize Fonte: Elaborado pelos autores com auxílio do Claude.}
\end{figure}
\FloatBarrier
